...
function onFileDone(file, intervalId) {
    // 1. Stop the progress interval if provided
    if (intervalId) {
        clearInterval(intervalId);
    }

    document.querySelector("#remaining").innerHTML = "Done !!!";
    document.querySelector("#uploadSpeed").innerHTML = "0";
    document.querySelector("#downloadSpeed").innerHTML = "0";
    document.querySelector("#downloaded").innerHTML = prettyBytes(parseInt(ANCHOR.getParams().id))
    document.querySelector("#progressBar").style.width = Math.round(100) + '%';

    file.appendTo("#output")
    
    // 2. Post to server (use file.length from the argument)
    $.post("/snatched/" + file.length);
    
    // 3. Add file to the queue (DO NOT APPEND DIRECTLY HERE!)
    //filesDownloaded.push(file);

    // 4. Attempt to process the queue and append files in order
...
// --- MAIN ENTRY POINT ---
function selectFile() {
    const targetLength = parseInt(ANCHOR.getParams().id);

    if (ANCHOR.getParams() && ANCHOR.getParams().media === "Ebook") {
    // 1. Deselect EVERYTHING first (Good practice, ensuring only one file is active)
      ...
      //TODO: sort files
      } else if (ANCHOR.getParams() && ANCHOR.getParams().media === "Audiobook") {
        const id = parseInt(ANCHOR.getParams().id);
        const arrAudio = [];
